...
$.allTask = [];
$.info = {};
let superInvites = [`6BBF95F47F9D67C85E37CB0C07EF16739187F4A21AA68306C50328C57AF5D2E9`,`CF2A744AA5E1FAED66179235F3DC8AA85563E27D559E0643E6E49523E8CE57DF`,`B245A72D8F320B2477D78F5D89AA97A92E3590A34149DC0FC55745C44F682995`,`624E80FA38762B73E86334A0EAE0CFDD49E90EA7F310083CEB5B724CFE26B771`];
let groupInvites = {};
...
function joinGroup() {
  return new Promise( async (resolve ) => {
    if(!groupInvites[`201116391-607759`]) return resolve();              
        $.get(taskUrl(`user/JoinGroup`, `strGroupId=${groupInvites[`201116391-607759`]}&dwIsNewUser=${$.info.dwIsNewUser}&pgtimestamp=${$.currentToken['timestamp']}&phoneID=${$.currentToken['phoneid']}&pgUUNum=${$.currentToken['farm_jstoken']}`), (err, resp, data) => {
          try {
            const { sErrMsg } = JSON.parse(data);
            $.log(`\n【🏝出海寻宝】助力：${sErrMsg}\n${$.showLog ? data : ''}`);            
          } catch (e) {
