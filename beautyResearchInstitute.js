...
只支持Node.js支持N个京东账号
脚本兼容: Node.js
cron 1 7,12,19 * * * beautyResearchInstitute.js
 */
const $ = new Env('玩一玩_美丽研究院');
...
async function mr() {
  ...
  client.onopen = async () => {
    console.log(`美容研究院服务器连接成功`);
    client.send(
      '{"msg":{"type":"action","args":{"source":1},"action":"_init_"}}'
    );
    // console.log(`accountCheck:${$.accountCheck}`);
      client.send(
        `{"msg":{"type":"action","args":{"source":"meizhuangguandibudaohang"},"action":"stats"}}`
      );
      while (!$.init) {
        client.send(`ping`);
        await $.wait(1000);
      ...
      // 获得可生产的原料列表
      client.send(
        `{"msg":{"type":"action","args":{},"action":"get_produce_material"}}`
      );
      await $.wait(1000);
      // 获得正在生产的商品信息
      client.send(
        '{"msg":{"type":"action","args":{},"action":"product_producing"}}'
      );
      await $.wait(1000);
      // 获得库存
      client.send(`{"msg":{"type":"action","args":{},"action":"get_package"}}`);
      // 获得可生成的商品列表
      console.log(`========获得可生成的商品列表========`);
      client.send(
        `{"msg":{"type":"action","args":{"page":1,"num":10},"action":"product_lists"}}`
      );
      await $.wait(1000);
      ...
      for (let pos of positionList) {
        client.send(
          `{"msg":{"type":"action","args":{"position":"${pos}"},"action":"produce_position_info_v2"}}`
        );
        // await $.wait(500)
      }
      ...
      // 获取个人信息
      client.send(
        `{"msg":{"type":"action","args":{"source":1},"action":"get_user"}}`
      );
      await $.wait(1000);
      // 获得福利中心
    ...
    if (e.data !== 'pong' && e.data && safeGet(e.data)) {
      ...
      switch (vo.action) {
        ...
        case "material_fetch_v2":
          if (vo.code === '200' || vo.code === 200) {
            // console.log(vo)
            console.log(`【${vo.data.position}】收取成功`)
          } else {
            console.log(`任务完成失败，错误信息${vo.msg}`)
          }
          break
