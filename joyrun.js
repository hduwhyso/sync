...
1.docker下填写京东用户名后直接用，不需要token。
2.同个容器或者圈x下的账号自动互相助力

===================Quantumult X=====================
[task_local]
# 宠汪汪邀请助力与赛跑助力
15 10,17 * * * https://raw.githubusercontent.com/hduwhyso/sync/main/joyrun.js, tag=宠汪汪邀请助力与赛跑助力, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true
...
//此处填入你需要助力好友的京东用户名
//下面给出好友邀请助力的示例填写规则
let invite_pins = []; //手动填写格式["jd_julBEausqrhE","jd_780ac31fe307e"]
//下面给出好友赛跑助力的示例填写规则
let run_pins = [];  //手动填写格式["jd_julBEausqrhE","jd_780ac31fe307e"]
...
async function main() {
 ...
 for (let i = 0; i < cookiesArr.length; i++) {
  if (cookiesArr[i]) {
    ...
    invite_pins.push(`${UserName}`);
    run_pins.push(`${UserName}`);    
    }    
  }
